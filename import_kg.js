/**
 * Importa dados da KG Odontologia Integrada do backup_kg.json
 * para o banco novo (u410205205_gerencie em srv1722.hstgr.io).
 *
 * Run: node scripts/import_kg.js
 */

const { Sequelize, DataTypes } = require('sequelize');
const { v4: uuidv4 } = require('uuid');
const path = require('path');
const backup = require('../backup_kg.json');

// ── Constantes do usuário KG no banco NOVO ─────────────────────────────────
const NOVO_USER_ID    = '26c3cd2c-a187-4e33-91a7-e6c51ffdbe3a';
const NOVO_CLINICA_ID = 'ec36f91b-0d18-49d7-b7fe-52e1a489a63e';
...
async function run() {
  ...
  let pacienteMap = {}; // old_id -> novo_uuid
  let procMap = {};     // old_id -> novo_uuid
  ...
  for (const p of backup.pacientes) {
    ...
    // Verificar se já existe (pelo CPF + clinica)
    if (p.cpf) {
  ...
  for (const r of backup.rendimentos) {
    // Verificar duplicata: mesmo valor + data + user + descrição
    const [existing] = await seq.query(
    ...
    const pacNome = r.paciente_id && pacientesById[r.paciente_id]
      ? pacientesById[r.paciente_id].nome
      : 'Paciente não identificado';
    const pacCpf = r.paciente_id && pacientesById[r.paciente_id]
      ? pacientesById[r.paciente_id].cpf
      : null;
      ...
      await seq.query(`
        INSERT INTO faturamentos
          (id, descricao, valor, data, forma_pagamento, paciente, cpf, tipo_pessoa,
           paciente_id, recibo_url, user_id, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, 'PF', ?, ?, ?, NOW(), NOW())
      `, {
        replacements: [
          uuidv4(),
          ...
          r.valor,
          r.data,
          mapFormaPagamento(r.forma_pagamento),
          pacNome,
          pacCpf || r.cpf || null,
          pacienteIdNovo,
          r.recibo || null,
  ...
  // ─── Resumo ─────────────────────────────────────────────────────────────
  console.log('═══════════════════════════════════════');
  console.log('MIGRAÇÃO KG ODONTOLOGIA — CONCLUÍDA');
  console.log('═══════════════════════════════════════');
